...
논문에 오타가 많으니 직접 읽어보실 분들은 주의하시길 바랍니다 !
...
위의 조건들을 점검해보면  $\mu_k>0$이면 $\xi=0$ 이어야 하므로 동시에 $\mu_k>0 ,\;\mu_k\xi_k>0$은 불가능하다. <br>
...
두 번째 implict constraint를 얻을 수 있다.
...
여기서 아쉬운 점은 이 방법론에서 가장 중요한 부분인데도 설명이 다소 빈약하게 느껴졌다.
kernel을 선택하는 데 어떤 이론적인 배경이 없는 것처럼 membership function도 마찬가지라고 생각한다..!
...
논문에서는 SVM, fuzzy SVM, new fuzzy SVM을 kernel과 membership의 조합에 따라 모델 성능을 비교햐고 있다. <br>
